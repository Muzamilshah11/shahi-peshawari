<?php
$pageTitle = 'Royal Premium Collection';
require_once 'config.php';
require_once 'product_data.php';
require_once 'header.php';

// Filter premium products
$premiumProducts = array_values(array_filter($PRODUCTS, function($p) {
    return ($p['cat'] ?? '') === 'premium';
}));
?>
...
<div class="container">
    ...
    <div class="breadcrumb" style="margin-bottom:14px;">
        ...
        <span>Royal Premium Collection</span>
    </div>

    <!-- Royal Hero Banner -->
    <div style="background:linear-gradient(135deg,#121212 0%,#241e15 50%,#3d301b 100%);border-radius:var(--radius);padding:40px 32px;color:#fff;margin-bottom:28px;position:relative;overflow:hidden;box-shadow:0 8px 30px rgba(0,0,0,.3);border:1px solid rgba(212,160,23,.3);">
        <div style="position:absolute;right:-20px;bottom:-20px;opacity:.06;font-size:14rem;pointer-events:none;color:#ffd700;"><i class="fas fa-crown"></i></div>
        <div style="max-width:620px;position:relative;z-index:1;">
            <div style="display:inline-flex;align-items:center;gap:6px;padding:5px 14px;background:rgba(212,160,23,.2);border:1px solid rgba(212,160,23,.5);color:#ffd700;border-radius:50px;font-size:.76rem;font-weight:700;text-transform:uppercase;letter-spacing:.8px;margin-bottom:12px;">
                <i class="fas fa-crown"></i> Royal Heritage Luxury
            </div>
            <h1 style="font-size:2.2rem;font-weight:800;margin-bottom:10px;line-height:1.2;background:linear-gradient(135deg,#fff 0%,#ffd700 100%);-webkit-background-clip:text;-webkit-text-fill-color:transparent;">Mastercrafted for Kings & Dignitaries</h1>
            <p style="font-size:.9rem;color:#d1ccc0;line-height:1.7;">The pinnacle of Peshawari shoe craftsmanship. Each pair features top-grain selected calf leather, lustrous mirror finish, gold & antique brass buckles, and dual-layer handcrafted tyre soles for grand occasions and wedding celebrations.</p>
        </div>
    </div>
    ...
    <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:16px;margin-bottom:32px;" class="trust-grid">
        <div style="background:#fff;border-radius:var(--radius);padding:20px;display:flex;align-items:center;gap:14px;box-shadow:var(--shadow);border:1px solid var(--grey-border);">
            <div style="width:48px;height:48px;border-radius:12px;background:linear-gradient(135deg,#d4a017,#f9ca24);display:flex;align-items:center;justify-content:center;color:#fff;font-size:1.3rem;flex-shrink:0;">
                <i class="fas fa-gem"></i>
            </div>
            <div>
                <h4 style="font-size:.92rem;font-weight:700;color:var(--black);">Top-Grade Calf Leather</h4>
                <p style="font-size:.76rem;color:var(--grey-text);margin-top:2px;">Hand-selected flawless hide</p>
            </div>
        </div>
        <div style="background:#fff;border-radius:var(--radius);padding:20px;display:flex;align-items:center;gap:14px;box-shadow:var(--shadow);border:1px solid var(--grey-border);">
            <div style="width:48px;height:48px;border-radius:12px;background:linear-gradient(135deg,#1e272e,#485460);display:flex;align-items:center;justify-content:center;color:#fff;font-size:1.3rem;flex-shrink:0;">
                <i class="fas fa-award"></i>
            </div>
            <div>
                <h4 style="font-size:.92rem;font-weight:700;color:var(--black);">High-Shine Mirror Finish</h4>
                <p style="font-size:.76rem;color:var(--grey-text);margin-top:2px;">Polished to radiant elegance</p>
            </div>
        </div>
        <div style="background:#fff;border-radius:var(--radius);padding:20px;display:flex;align-items:center;gap:14px;box-shadow:var(--shadow);border:1px solid var(--grey-border);">
            <div style="width:48px;height:48px;border-radius:12px;background:linear-gradient(135deg,#b71540,#e55039);display:flex;align-items:center;justify-content:center;color:#fff;font-size:1.3rem;flex-shrink:0;">
                <i class="fas fa-ring"></i>
            </div>
            <div>
                <h4 style="font-size:.92rem;font-weight:700;color:var(--black);">Wedding & Festive Ready</h4>
                <p style="font-size:.76rem;color:var(--grey-text);margin-top:2px;">Statement elegance for special days</p>
            </div>
        </div>
    </div>
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px;flex-wrap:wrap;gap:10px;">
        <div>
            <h2 style="font-size:1.35rem;font-weight:800;color:var(--black);display:flex;align-items:center;gap:8px;">
                <i class="fas fa-crown" style="color:#d4a017;"></i> Royal Luxury Edition
            </h2>
            <p style="font-size:.82rem;color:var(--grey-text);margin-top:2px;">Available in <?= count($premiumProducts) ?> distinguished mastercraft editions</p>
        </div>
        <div style="font-size:.8rem;color:#b7791f;font-weight:600;background:#fff8e6;padding:6px 14px;border-radius:50px;border:1px solid #ffeeba;">
            <i class="fas fa-certificate"></i> Certified Premium Leather
        </div>
    </div>

    <!-- Product Grid -->
    <div class="product-grid" id="premiumGrid" style="margin-bottom:48px;">
        ...
        ?>
        <div class="product-card" data-id="<?= $p['id'] ?>" data-name="<?= $pName ?>" data-cat="premium" style="border:1px solid rgba(212,160,23,.2);position:relative;">
            
            <div style="position:absolute;top:10px;left:10px;z-index:5;background:linear-gradient(135deg,#d4a017,#b7791f);color:#fff;font-size:.66rem;font-weight:700;padding:3px 9px;border-radius:50px;letter-spacing:.5px;box-shadow:0 2px 6px rgba(0,0,0,.15);">
                👑 ROYAL EDITION
            </div>
            ...
            <div class="card-body">
                <span class="card-cat" style="color:#d4a017;font-weight:700;">Royal Heritage</span>
                ...
                <div class="card-price" style="color:#b7791f;">Rs.<?= number_format($price) ?>
...
<div class="modal-overlay" id="quickBuyModal">
    <div class="modal-content">
        <div class="modal-header">
            <h2><i class="fas fa-shopping-bag"></i> Quick Order - Royal Premium</h2>
            ...
            <form onsubmit="return false;">
                ...
                <div class="form-group"><label><i class="fas fa-map-marker-alt" style="color:var(--blue)"></i> Address</label>
                    <div style="display:flex;gap:8px;align-items:center;">
                        <input type="text" id="qbAddress" placeholder="Enter delivery address" style="flex:1">
                    </div>
                </div>
                <div class="form-row">
    ...
    db.ref('products').on('value', function(snap) {
        try {
            ...
            Object.keys(fbProducts).forEach(function(id) {
                ...
                html += '<div class="product-card" data-id="' + id + '" data-name="' + safeName + '" data-cat="premium" style="border:1px solid rgba(212,160,23,.2);position:relative;">'
                    + '<div class="img-wrap"><img src="' + src1 + '" alt="' + safeName + '" loading="lazy" onerror="this.onerror=null;this.src=\'' + PLACEHOLDER + '\'">'
                    + '<button class="heart-btn" data-id="' + id + '" data-name="' + safeName + '" data-price="' + raw.price + '" data-img="' + src1 + '" title="Add to Cart"><i class="fas fa-heart"></i></button></div>'
                    + '<div class="card-body"><span class="card-cat">premium</span><h3 class="card-title">' + safeName + '</h3><div style="margin:3px 0">' + stars + '</div>'
                    ...
                    + '<button class="btn-buy-card" data-id="' + id + '" data-name="' + safeName + '" data-price="' + raw.price + '" data-imgs="' + src1 + '|' + src2 + '"><i class="fas fa-bolt"></i> Buy Now</button>'
                    + '</div></div>';
            });
