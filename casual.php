<?php
$pageTitle = 'Casual & Everyday Comfort Collection';
require_once 'config.php';
require_once 'product_data.php';
require_once 'header.php';

// Filter casual products
$casualProducts = array_values(array_filter($PRODUCTS, function($p) {
    return ($p['cat'] ?? '') === 'casual';
}));
?>
...
<div class="container">
    ...
    <div class="breadcrumb" style="margin-bottom:14px;">
        ...
        <span>Casual & Everyday Collection</span>
    </div>

    <!-- Casual Hero Banner -->
    <div style="background:linear-gradient(135deg,#2c3e50 0%,#34495e 50%,#2980b9 100%);border-radius:var(--radius);padding:38px 30px;color:#fff;margin-bottom:28px;position:relative;overflow:hidden;box-shadow:var(--shadow-lg);">
        <div style="position:absolute;right:-20px;bottom:-20px;opacity:.08;font-size:13rem;pointer-events:none;"><i class="fas fa-cloud-sun"></i></div>
        <div style="max-width:620px;position:relative;z-index:1;">
            <div style="display:inline-flex;align-items:center;gap:6px;padding:5px 14px;background:rgba(0,184,148,.2);border:1px solid rgba(0,184,148,.4);color:#55efc4;border-radius:50px;font-size:.76rem;font-weight:700;text-transform:uppercase;letter-spacing:.8px;margin-bottom:12px;">
                <i class="fas fa-feather"></i> Everyday Comfort First
            </div>
            <h1 style="font-size:2.1rem;font-weight:800;margin-bottom:10px;line-height:1.2;">Effortless Comfort for Daily Steps</h1>
            <p style="font-size:.9rem;color:#dfe6e9;line-height:1.7;">Crafted for long days on your feet. Featuring supple genuine suede, breathable cutout patterns, open-toe slide designs, and cushioned footbeds that mold to your foot.</p>
        </div>
    </div>
    ...
    <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:16px;margin-bottom:32px;" class="trust-grid">
        <div style="background:#fff;border-radius:var(--radius);padding:20px;display:flex;align-items:center;gap:14px;box-shadow:var(--shadow);border:1px solid var(--grey-border);">
            <div style="width:48px;height:48px;border-radius:12px;background:linear-gradient(135deg,#00b894,#55efc4);display:flex;align-items:center;justify-content:center;color:#fff;font-size:1.3rem;flex-shrink:0;">
                <i class="fas fa-shoe-prints"></i>
            </div>
            <div>
                <h4 style="font-size:.92rem;font-weight:700;color:var(--black);">Padded Comfort Footbed</h4>
                <p style="font-size:.76rem;color:var(--grey-text);margin-top:2px;">Soft cushioning for fatigue-free walking</p>
            </div>
        </div>
        <div style="background:#fff;border-radius:var(--radius);padding:20px;display:flex;align-items:center;gap:14px;box-shadow:var(--shadow);border:1px solid var(--grey-border);">
            <div style="width:48px;height:48px;border-radius:12px;background:linear-gradient(135deg,#e17055,#fab1a0);display:flex;align-items:center;justify-content:center;color:#fff;font-size:1.3rem;flex-shrink:0;">
                <i class="fas fa-wind"></i>
            </div>
            <div>
                <h4 style="font-size:.92rem;font-weight:700;color:var(--black);">Breathable Airy Design</h4>
                <p style="font-size:.76rem;color:var(--grey-text);margin-top:2px;">Artistic cutouts & open-toe slides</p>
            </div>
        </div>
        <div style="background:#fff;border-radius:var(--radius);padding:20px;display:flex;align-items:center;gap:14px;box-shadow:var(--shadow);border:1px solid var(--grey-border);">
            <div style="width:48px;height:48px;border-radius:12px;background:linear-gradient(135deg,#0984e3,#74b9ff);display:flex;align-items:center;justify-content:center;color:#fff;font-size:1.3rem;flex-shrink:0;">
                <i class="fas fa-shield-halved"></i>
            </div>
            <div>
                <h4 style="font-size:.92rem;font-weight:700;color:var(--black);">Flexible Tyre/Rubber Sole</h4>
                <p style="font-size:.76rem;color:var(--grey-text);margin-top:2px;">Long-lasting anti-slip durability</p>
            </div>
        </div>
    </div>
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px;flex-wrap:wrap;gap:10px;">
        <div>
            <h2 style="font-size:1.35rem;font-weight:800;color:var(--black);display:flex;align-items:center;gap:8px;">
                <i class="fas fa-leaf" style="color:var(--green);"></i> Casual & Everyday Lineup
            </h2>
            <p style="font-size:.82rem;color:var(--grey-text);margin-top:2px;">Showing all <?= count($casualProducts) ?> casual handcrafted chappals</p>
        </div>
        <div style="font-size:.8rem;color:var(--green);font-weight:600;background:#e6fff9;padding:6px 14px;border-radius:50px;">
            <i class="fas fa-thumbs-up"></i> Best for Summer & Daily Wear
        </div>
    </div>
    ...
    <div class="product-grid" id="casualGrid" style="margin-bottom:48px;">
        ...
        ?>
        <div class="product-card" data-id="<?= $p['id'] ?>" data-name="<?= $pName ?>" data-cat="casual" style="position:relative;">
            
            <div style="position:absolute;top:10px;left:10px;z-index:5;background:rgba(0,184,148,.9);color:#fff;font-size:.66rem;font-weight:700;padding:3px 9px;border-radius:50px;letter-spacing:.5px;backdrop-filter:blur(4px);">
                COMFORT WEAR
            </div>
            ...
            <div class="card-body">
                <span class="card-cat" style="color:var(--green);font-weight:700;">Casual & Suede</span>
...
<div class="modal-overlay" id="quickBuyModal">
    <div class="modal-content">
        <div class="modal-header">
            <h2><i class="fas fa-shopping-bag"></i> Quick Order - Casual Collection</h2>
